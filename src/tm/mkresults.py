...
PATTERN_KINDS = SIGNATURE, SOURCE, SOURCES, CASE, CASES = (
    "signature",
    "source",
    "sources",
    "case",
...
class FileSystemScanner(object, metaclass=ScannerTracker):
    ...
    def default_reader(self, path):
        with open(path, "rb") as f:
            content = str(f.read(), errors="replace")
        return content
    ...
    def __init__(self, basedir, *args, **kwargs):
        ...
        if LOGGER.isEnabledFor(DEBUG):
            ...
            for kind, pattern in list(self.patterns.items()):
                LOGGER.debug(
                    "Kind {0}, pattern = {1}, reader = {2}".format(
    ...
    def scan(self):
        def _to_keys(match):
            ...
            if not keys.uid:
                raise RuntimeError('Match must contains at least "uid"')
            return keys
        ...
        def _assign(kind, keys, content):
            ...
            if kind == SIGNATURE:
                ...
                results[keys.uid]["exercises"][keys.exercise]["sources"][
                    keys.source
                ] = content
            elif kind == SOURCES:
                results[keys.uid]["exercises"][keys.exercise]["sources"] = content
            elif kind == CASE:
                results[keys.uid]["exercises"][keys.exercise]["cases"][
                    keys.case
                ] = content
            elif kind == CASES:
                results[keys.uid]["exercises"][keys.exercise]["cases"] = content
            else:
                raise RuntimeError('Unkown kind: "{0}"'.format(kind))
            ...
            for name in files:
                ...
                for kind, pattern in list(self.patterns.items()):
                    ...
                    if match:
                        keys = _to_keys(match)
                        if keys.exercise:
                            exercise_names.add(keys.exercise)
        ...
        self.results = []
        for uid, res in list(results.items()):
            exercises = []
    ...
    def tojson(self, accept=None):
        if accept is None:
            ...
            return dumps(list(filter(accept, self.results)))
...
def main():
    ...
    parser.add_argument(
        "--scanner",
        "-s",
        help="The scanner to use (default: {0}; available: {1})".format(
            SCANNERS[-1][0], ", ".join(map(itemgetter(0), SCANNERS))
        ),
    )
    parser.add_argument(
        "results_dir", help="The directory where the results where stored"
    )
    parser.add_argument(
        "json_output",
        help="The file where to store the json summary",
        type=FileType("wb"),
    )
    parser.add_argument(
        "extra_args", help="Extra arguments, passed to the scanner", nargs=REMAINDER
    )
    ...
    if "." in args.scanner:
        ...
        try:
            ...
            parser.error(
                'Scanner "{0}" not found; use -h to obtains a list of available scanners.'.format(
                    args.scanner
                )
            )

    args.json_output.write(
        scanner(args.results_dir, *args.extra_args).scan().sort().tojson()
    )
